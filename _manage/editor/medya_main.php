...
return $ayx;
	...
	echo"---------------------------$orderdate";
		...
		$orderdate = "$ord[2]-$ord[1]-$ord[0]";
			...
			mysql_query("insert into medya (tarih, yayin, keywords, zaman, editor) values ('$orderdate','$yayin','$keywords','$zaman','$editor')")or die("DB Error");
...
$ressim = resim($filename_name);
  ...
  if($filename){
	...
	mysql_query("update medya set foto_1='$home_sql_name', foto_2='$detail_sql_name', foto_3='$large_sql_name' where id='$giris'");
...
echo "<script> alert(\"Fotograf galeriye eklendi $home_sql_name / $galeri...!\"); </script>";
	...
	mysql_query("insert into loglar (islem,editorid,eusername,ipno,zaman) values ('$proc','$my_eid','$e_uname','$e_ip','$tarih')");
		...
		mysql_query("delete from medya where id='$remhid'");
...
echo "<script> alert(\"Data silindi...!\"); </script>";
...
$welcomefile=$docroot."/admin/_manage/editor/medya_main.src.php";
include($welcomefile);
}
