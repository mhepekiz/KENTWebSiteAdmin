...
$langfile=$docroot."/admin/_manage/editor/includes/lang.php";
	...
	$get_link = mysql_query("select id, lang, linkadi, linkadres, linkaciklama from linkler where id='$remhid'");
	list($link_id, $lang, $link_ad, $link_url, $link_detay) = mysql_fetch_row($get_link);
...
<form id="myform" name="myform" class="fValidator-form"  method="POST" enctype="multipart/form-data" class="fValidator-form" action="/admin/_manage/editor/link_edit.php" >
  ...
  <table border="0" cellpadding="2" style="border-collapse: collapse" bordercolor="#111111" width="100%" id="AutoNumber1" cellpadding="5">
    ...
    <tr>
      ...
      <?
	  	...
	  	if($lang=="en"){ echo'<option selected value="en">İngilizce</option>'; }
		if($lang=="ar"){ echo'<option selected value="ar">Arapça</option>'; }
		if($lang=="ge"){ echo'<option selected value="ge">Gürcüce</option>'; }
		if($lang=="az"){ echo'<option selected value="az">Azerice</option>'; } 
    ...
    <td width="90%"><input type="text" name="linkurl" size="70" value="<?=$link_url?>"></td>
    </tr> 
	<tr>
	<td align="left">Link Ad</td>
    <td width="90%"><input type="text" name="linkad" size="70" value="<?=$link_ad?>"></td>
    </tr> 
    
    <tr>
	<td align="left">Link Detay</td>
    <td width="90%"><input type="text" name="linkdetay" size="70" value="<?=$link_detay?>"></td>
    </tr> 
...
<tr valign="top">  <td width="100%" colspan="2"><table width="100%" border="0" cellpadding="3">
 <tr>
  ...
  <input type="hidden" name="remhid" value="<?=$link_id?>">
...
$get_linx = mysql_query("select id,linkadi,linkadres,zaman,editor from linkler $newsq");
while(list($linkid, $linkadi, $linkurl, $ltarih, $leditor)=mysql_fetch_row($get_linx)){
...
<td bgcolor="'.$bgc.'" nowrap align="center"><a href="/admin/_manage/editor/link_edit.php?ac_t=update&remhid='.$linkid.'"><img src="/admin/_media/edit.gif" border="0"></a></td>
		...
		<td bgcolor="'.$bgc.'" nowrap width="100%">'.$linkadi.'</td>
