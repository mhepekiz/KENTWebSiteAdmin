...
if($ac_t=="upd"){
	...
	//$metin = mysql_real_escape_string($metin);
		...
		//	$baslik = addslashes($baslik);
			...
			mysql_query("update poliklinik_alt set baslik='$ekbaslik', detay='$metin', zaman='$zaman', editor='$e_uname' where id='$remhid'")or die("DB Error");
...
echo "<script> alert(\"Poliklinik detay guncellendi ($ekbaslik)!\"); </script>";
	...
	mysql_query("delete from haberler where id='$remhidx'");		
...
$welcomefile=$docroot."/admin/_manage/editor/add2pol_edit.src.php";
include($welcomefile);
}
