...
$pitcure_path = $docroot."/_all_media/_ortak/";
$pitcure_folder = "/_all_media/_ortak/";
  ...
  if($filename){
	...
	rename("/var/www/vhosts/2012.hepekiz.com/httpdocs/_all_media/_ortak/$filename_name", "/var/www/vhosts/2012.hepekiz.com/httpdocs/$sql_name_large");
	...
	mysql_query("insert into ortakbanner (banner,zaman,editor) values ('$sql_name_large','$zaman','$e_uname')") or die("DB Err");
...
echo "<script> alert(\"Banner eklendi ($banner)!\"); </script>";
	...
	mysql_query("insert into loglar (islem,editorid,eusername,ipno,zaman) values ('$proc','$my_eid','$e_uname','$e_ip','$tarih')");
		...
		if($ac=t=="del"){
				...
				$get_ban = mysql_query("select id, banner from ortakbanner where id='$remhid'");
...
$welcomefile=$docroot."/admin/_manage/editor/isortaklari.src.php";
include($welcomefile);
