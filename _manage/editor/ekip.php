...
if($ac_t=="add"){
	...
	$metin = mysql_real_escape_string($metin);
		...
		//	$baslik = addslashes($baslik);
			...
			mysql_query("insert into ekip (adsoyad, pozisyon, klinik, ozgecmis, zaman, editor) values ('$adsoyad','$pozisyon','$klinik','$metin','$zaman','$e_uname')")or die("DB Error");
...
$filename_name = resim($filename_name);
	...
	mysql_query("update ekip set foto_1='$home_sql_name', foto_2='$detail_sql_name' where id='$ekipuye'") or die("DB Err");
...
//echo"$home_sql_name//////// $large_sql_name";
  ...
  //  unlink($moved);
		...
		if($ac_t=="del"){
				...
				$get_adam = mysql_query("select id, adsoyad from ekip where id='$remhid'");
				list($e_id, $e_ad) = mysql_fetch_row($get_adam);

			
		
		mysql_query("delete from ekip where id='$remhid'");
		
			   
$proc = "Doktor silindi ($e_ad)...";



echo "<script> alert(\"Doktor silindi ($e_ad)!\"); </script>";
...
$welcomefile=$docroot."/admin/_manage/editor/ekip.src.php";
include($welcomefile);
